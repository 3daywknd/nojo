/**
 * Tests for nojo-prune-context intercepted slash command
 *
...
describe.skip("nojo-prune-context [NEEDS REFACTOR - slash command behavior changed]", () => {
  let testDir: string;
  ...
  describe("matchers", () => {
    it("should have valid regex matchers", () => {
      expect(nojoPruneContext.matchers).toBeInstanceOf(Array);
      expect(nojoPruneContext.matchers.length).toBeGreaterThan(0);

      for (const matcher of nojoPruneContext.matchers) {
        expect(() => new RegExp(matcher)).not.toThrow();
      }
    });

    it("should match /nori-prune-context", () => {
      const regex = new RegExp(nojoPruneContext.matchers[0]);
  ...
  describe("run function", () => {
    it("should return block decision", async () => {
      ...
      const result = await nojoPruneContext.run({ input });
  ...
  describe("pruning behavior", () => {
    ...
    it("should preserve other settings fields", async () => {
      ...
      const input = createInput({ prompt: "/nori-prune-context" });
      await nojoPruneContext.run({ input });
  ...
  describe("error handling", () => {
    it("should handle malformed JSON gracefully", async () => {
      ...
      // Should not throw
      await expect(nojoPruneContext.run({ input })).resolves.not.toThrow();
    });
  });
});
