/**
 * Tests for nojo-switch-profile intercepted slash command
 */
...
describe.skip("nojo-switch-profile [NEEDS REFACTOR - slash command behavior changed]", () => {
  let testDir: string;
  ...
  beforeEach(async () => {
    ...
    configPath = path.join(testDir, ".nojo-config.json");
  ...
  describe("matchers", () => {
    it("should have valid regex matchers", () => {
      expect(nojoSwitchProfile.matchers).toBeInstanceOf(Array);
      expect(nojoSwitchProfile.matchers.length).toBeGreaterThan(0);

      for (const matcher of nojoSwitchProfile.matchers) {
  ...
  describe("run function", () => {
    it("should switch profile when profile name provided", async () => {
      ...
      const result = await nojoSwitchProfile.run({ input });
  ...
  describe("installDir resolution", () => {
    ...
    it("should return error when no installation found", async () => {
      // Create a directory with NO nojo installation markers
