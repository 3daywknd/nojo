 ...
 * Handles /nojo/switch-profile commands for instant profile switching
 */
...
import { checkAndSaveModifications } from "@/cli/features/claude-code/profiles/modifications.js";
...
/**
 * Run the nojo/switch-profile command
...
const run = async (args: { input: HookInput }): Promise<HookOutput | null> => {
  ...
  const profileMatch = trimmedPrompt.match(
    /^\/nojo\/switch-profile(?:\s+([a-z0-9-]+))?\s*$/i,
  );
  ...
  if (allInstallations.length === 0) {
    return {
      decision: "block",
      reason: formatError({ message: `No nojo installation found.` }),
    };
  }
  ...
  try {
    // Check for modifications to current profile and save them before switching
    const config = await loadConfig({ installDir });
    const currentProfile = config
      ? getAgentProfile({ config, agentName: "claude-code" })?.baseProfile
      : null;

    let modificationsSaved = false;
    if (currentProfile != null && currentProfile !== profileName) {
      // Save modifications to current profile before switching
      modificationsSaved = await checkAndSaveModifications({
        installDir,
    ...
    // Spawning nojo as a subprocess runs the CLI from its installed location
    // where paths resolve correctly.
    // See: https://github.com/evanw/esbuild/issues/1921
    execSync(
      `nojo install --non-interactive --silent --skip-uninstall --install-dir "${installDir}" --agent claude-code`,
    ...
    let message = `Profile switched to "${profileName}"${profileDescription ? `: ${profileDescription}` : ""}.`;
    if (modificationsSaved && currentProfile) {
      message += `\n\nYour modifications to "${currentProfile}" have been saved.`;
    }
...
/**
 * nojo/switch-profile intercepted slash command
 */
export const nojoSwitchProfile: InterceptedSlashCommand = {
  matchers: [
    "^\\/nojo\\/switch-profile\\s*$",
    "^\\/nojo\\/switch-profile\\s+[a-z0-9-]+\\s*$",
  ],
  run,
};
