/**
 * Tests for nojo-install-location intercepted slash command
 */
...
describe("nojo-install-location", () => {
  let testDir: string;
  ...
  beforeEach(async () => {
    ...
    // Create config file to mark as nojo installation
    await fs.writeFile(
      configPath,
      JSON.stringify({
        profile: {
  ...
  describe("matchers", () => {
    it("should have valid regex matchers", () => {
      expect(nojoInstallLocation.matchers).toBeInstanceOf(Array);
      expect(nojoInstallLocation.matchers.length).toBeGreaterThan(0);

      for (const matcher of nojoInstallLocation.matchers) {
  ...
  describe("behavior", () => {
    it("should return installation directory", async () => {
      ...
      expect(plainReason).toContain("nojo installation");
    });
