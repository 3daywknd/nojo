...
- **Separate from profile slash commands**: Profile-specific commands (like `nori-init-docs`, `nori-sync-docs`) remain in profile mixins at @/src/cli/features/claude-code/profiles/config/_mixins/ and are handled by the profile slashcommands loader at @/src/cli/features/claude-code/profiles/slashcommands/loader.ts
